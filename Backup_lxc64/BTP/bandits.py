...
class PFEDUCB(object):
	def __init__(self,
		...
		# print("globa_means =",self.global_means)
	...
	def simulate_single_step(self, plays):
		...
		local_rewards = np.array([local_rews[i, plays[i]]
	...
	def simulate(self):
		...
		for t in range(self.T):
			...
			for i in range(self.M):
				# update strategies of all player
				self.clients[i].reward_update(plays[i], local_rews[i])
		...
		for i in range(self.M):
			top_arms[i] = np.argmax(
				self.alpha*self.local_means[i]+(1-self.alpha)*self.global_means)
			top_mixed_means[i] = self.alpha*self.local_means[i][top_arms[i]] + (1-self.alpha)*self.global_means[top_arms[i]]*self.weights[i]
			sub_gap[i] = np.sort(self.alpha*self.local_means[i] +
								 (1-self.alpha)*self.global_means)[-2]-top_mixed_means[i]
